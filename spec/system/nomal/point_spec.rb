...
describe 'ポイント機能' do
  ...
  describe '一覧' do
    it '項目が全て表示されている' do
      ...
      expect(page).to have_content "#{ point.get_point }pt"
    end
  ...
  describe '詳細' do
    ...
    it '項目が表示されている' do
      ...
      expect(page).to have_content "#{ admin.name }から#{ special_point.get_point }"
      expect(page).to have_content point_notice.detail
      expect(page).to have_content strftime('%Y/%m/%d %H:%M')
    end

    context '特別ポイントの場合' do
      let(:special_point) { FactoryBot.create(:special_point, message: 'がんばった', room_id: room.id, user_id: user.id) }
      let(:point_notice) { FactoryBot.create(:point_notice, room_id: room.id, user_id: user.id, special_point_id: special_point.id) }
